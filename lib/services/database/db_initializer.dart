...
import 'migrations/capital_social_migrations.dart';
...
class DatabaseInitializer {
  ...
  static Future<Database> _initDatabase() async {
    ...
    // Vérifier que les tables du module Capital Social existent (sécurité supplémentaire)
    await _ensureCapitalSocialTablesExist(database);

    return database;
  }

  /// S'assurer que les tables du module Capital Social existent
  ///
  /// Certaines bases existantes peuvent ne pas avoir exécuté la migration
  /// correspondante (ou avoir un historique de versions différent).
  /// On garantit ici l'existence de `souscriptions_capital` (et tables dépendantes)
  /// pour éviter les erreurs runtime dans le module Capital.
  static Future<void> _ensureCapitalSocialTablesExist(Database db) async {
    try {
      // 0) Toujours garantir la présence de la table de valeurs des parts
      // (ne dépend pas des autres tables du module Capital Social)
      await _ensurePartsSocialesValeursTable(db);

      final result = await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='souscriptions_capital'",
      );

      if (result.isEmpty) {
        print(
          '⚠️ Table souscriptions_capital introuvable, création en cours...',
        );
        await CapitalSocialMigrations.migrateToV18(db);

        final verification = await db.rawQuery(
          "SELECT name FROM sqlite_master WHERE type='table' AND name='souscriptions_capital'",
        );
        if (verification.isNotEmpty) {
          print('✅ Tables Capital Social créées avec succès');
        } else {
          print('❌ Échec de la création des tables Capital Social');
          throw Exception('Impossible de créer les tables Capital Social');
        }
      } else {
        // Optionnel : s'assurer que les index/valeur par défaut existent
        // (migrateToV18 utilise des CREATE TABLE/INDEX IF NOT EXISTS, donc c'est safe)
        await CapitalSocialMigrations.migrateToV18(db);
      }
    } catch (e, stackTrace) {
      print(
        '❌ Erreur lors de la vérification/création des tables Capital Social: $e',
      );
      print('Stack trace: $stackTrace');
      // Ne pas faire échouer l'initialisation complète
      try {
        print('🔄 Tentative de création forcée des tables Capital Social...');
        await CapitalSocialMigrations.migrateToV18(db);
        print('✅ Tables Capital Social créées avec succès (tentative forcée)');
      } catch (e2, stackTrace2) {
        print('❌ Impossible de créer les tables Capital Social: $e2');
        print('Stack trace: $stackTrace2');
      }
    }
  }

  static Future<void> _ensurePartsSocialesValeursTable(Database db) async {
    try {
      final existing = await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='parts_sociales_valeurs'",
      );

      if (existing.isEmpty) {
        print(
          '⚠️ Table parts_sociales_valeurs introuvable, création en cours...',
        );
        await db.execute('''
      ...
      // Garantir au moins une valeur active
      ...
      print('⚠️ Impossible d\'assurer parts_sociales_valeurs: $e');
  ...
  static Future<void> _ensureSocialTablesExist(Database db) async {
    try {
      ...
      print(
        '❌ Erreur lors de la vérification/création des tables sociales: $e',
      );
      print('Stack trace: $stackTrace');
  ...
  static Future<void> _ensureCommissionsTableExists(Database db) async {
    try {
      ...
      print(
        '❌ Erreur lors de la vérification/création de la table commissions: $e',
      );
      // Essayer de créer quand même
      try {
        await CommissionsModuleMigration.createCommissionsTables(db);
  ...
  static Future<void> _onCreate(Database db, int version) async {
    ...
    await db.execute(
      'CREATE INDEX idx_stock_depots_adherent ON stock_depots(adherent_id)',
    );
    ...
    await db.execute(
      'CREATE INDEX idx_recettes_adherent ON recettes(adherent_id)',
    );
    await db.execute(
      'CREATE INDEX idx_factures_adherent ON factures(adherent_id)',
    );
    await db.execute('CREATE INDEX idx_factures_numero ON factures(numero)');
    await db.execute(
      'CREATE INDEX idx_adherent_historique_adherent ON adherent_historique(adherent_id)',
    );
    await db.execute(
      'CREATE INDEX idx_adherent_historique_date ON adherent_historique(date_operation)',
    );
    await db.execute(
      'CREATE INDEX idx_adherents_village ON adherents(village)',
    );
    await db.execute(
      'CREATE INDEX idx_campagnes_active ON campagnes(is_active)',
    );
    await db.execute(
      'CREATE INDEX idx_campagnes_dates ON campagnes(date_debut, date_fin)',
    );
    await db.execute(
      'CREATE INDEX idx_baremes_qualite ON baremes_qualite(qualite)',
    );
    await db.execute(
  ...
  static Future<void> _onUpgrade(
    ...
    if (oldVersion < 2) {
      ...
      try {
        ...
        // Créer les index
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherent_historique_adherent ON adherent_historique(adherent_id)',
        );
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherent_historique_date ON adherent_historique(date_operation)',
        );
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherents_village ON adherents(village)',
        );
      } catch (e) {
    ...
    if (oldVersion < 3) {
      ...
      try {
        ...
        await db.execute(
          'ALTER TABLE stock_mouvements ADD COLUMN adherent_id INTEGER',
        );

        // Créer index pour adherent_id dans stock_mouvements
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_stock_mouvements_adherent ON stock_mouvements(adherent_id)',
        );
      } catch (e) {
    ...
    if (oldVersion < 4) {
      ...
      try {
        ...
        await db.execute(
          'ALTER TABLE ventes ADD COLUMN statut TEXT DEFAULT \'valide\'',
        );
      } catch (e) {
    ...
    if (oldVersion < 5) {
      ...
      try {
        ...
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_campagnes_active ON campagnes(is_active)',
        );
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_campagnes_dates ON campagnes(date_debut, date_fin)',
        );
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_baremes_qualite ON baremes_qualite(qualite)',
        );
      } catch (e) {
    ...
    if (oldVersion < 8) {
      ...
      try {
        ...
        print(
          'Erreur lors de la migration vers la version 8 (Adhérents Expert): $e',
        );
        rethrow;
      }
    }
    ...
    if (oldVersion < 12) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 12 (Module Ventes V1)...',
        );
    ...
    if (oldVersion < 13) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 13 (Module Ventes V2)...',
        );
    ...
    if (oldVersion < 14) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 14 (Intégration Adhérents ↔ Ventes)...',
        );
    ...
    if (oldVersion < 15) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 15 (Module Recettes Avancé)...',
        );
    ...
    if (oldVersion < 16) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 16 (Module Documents Officiels)...',
        );
    ...
    if (oldVersion < 17) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 17 (Module Clients)...',
        );
    ...
    if (oldVersion < 18) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 18 (Fusion Capital + Comptabilité)...',
        );
    ...
    if (oldVersion < 19) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 19 (Module de Paramétrage Complet)...',
        );
    ...
    if (oldVersion < 20) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 20 (Backend Multi-Coopérative)...',
        );
    ...
    if (oldVersion < 21) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 21 (Table settings complète)...',
        );
    ...
    if (oldVersion < 22) {
      ...
      try {
        print(
          'Exécution de la migration vers la version 22 (Système de rôles et permissions)...',
        );
    ...
    if (oldVersion < 23) {
      try {
        print(
          'Exécution de la migration vers la version 23 (Module Social)...',
        );
  ...
  static Future<void> _migrateToV9(Database db) async {
    ...
    Future<void> addColumnIfNotExists(
      ...
      try {
        await db.execute(
          'ALTER TABLE adherents ADD COLUMN $columnName $columnDefinition',
        );
        ...
        if (!e.toString().contains('duplicate column name') &&
            !e.toString().contains('already exists')) {
          print(
            '⚠️ Avertissement lors de l\'ajout de la colonne $columnName: $e',
          );
        }
      }
    }
    ...
    final columnNamesAfter = columnsAfter
    ...
    if (columnNamesAfter.contains('section')) {
      try {
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherents_section ON adherents(section)',
        );
      } catch (e) {
    ...
    if (columnNamesAfter.contains('site_cooperative')) {
      try {
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherents_site ON adherents(site_cooperative)',
        );
      } catch (e) {
    ...
    if (columnNamesAfter.contains('numero_piece')) {
      try {
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherents_numero_piece ON adherents(numero_piece)',
        );
      } catch (e) {
        print(
          '⚠️ Erreur lors de la création de idx_adherents_numero_piece: $e',
        );
      }
    }
    ...
    if (columnNamesAfter.contains('categorie')) {
      try {
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_adherents_categorie ON adherents(categorie)',
        );
      } catch (e) {
  ...
  static Future<void> _migrateToV10(Database db) async {
    ...
    Future<void> addColumnIfNotExists(
      ...
      try {
        await db.execute(
          'ALTER TABLE stock_depots ADD COLUMN $columnName $columnDefinition',
        );
  ...
  static Future<void> _ensureStockDepotsColumns(Database db) async {
    try {
      ...
      try {
        ...
        print(
          'Avertissement lors de la mise à jour des données existantes: $e',
        );
      }
    } catch (e) {
