...
class ChampParcelleService {
  ...
  Future<ChampParcelleModel> createChamp({
    ...
    try {
      ...
      print('🔍 Création champ - Paramètres reçus: latitude=$latitude, longitude=$longitude');
      print('🔍 Création champ - Données à insérer: latitude=${dataToInsert['latitude']}, longitude=${dataToInsert['longitude']}');
      print('🔍 Création champ - Modèle: latitude=${champ.latitude}, longitude=${champ.longitude}');
      ...
      if (verification.isNotEmpty) {
        ...
        print('🔍 Vérification DB - latitude=${verification.first['latitude']}, longitude=${verification.first['longitude']}');
      }
  ...
  Future<ChampParcelleModel> updateChamp({
    ...
    try {
      ...
      if (anneeMiseEnCulture != null) dataToUpdate['annee_mise_en_culture'] = anneeMiseEnCulture;
      if (etatChamp != null) dataToUpdate['etat_champ'] = etatChamp;
      if (rendementEstime != null) dataToUpdate['rendement_estime'] = rendementEstime;
      if (campagneAgricole != null) dataToUpdate['campagne_agricole'] = campagneAgricole;
      if (varieteCacao != null) dataToUpdate['variete_cacao'] = varieteCacao;
      if (nombreArbres != null) dataToUpdate['nombre_arbres'] = nombreArbres;
      if (ageMoyenArbres != null) dataToUpdate['age_moyen_arbres'] = ageMoyenArbres;
      if (densiteArbresAssocies != null) dataToUpdate['densite_arbres_associes'] = densiteArbresAssocies;
      if (systemeIrrigation != null) dataToUpdate['systeme_irrigation'] = systemeIrrigation;
      if (notes != null) dataToUpdate['notes'] = notes;
      
      print('🔍 Mise à jour champ ID $id - Paramètres reçus: latitude=$latitude, longitude=$longitude');
      print('🔍 Mise à jour champ - Données à mettre à jour: latitude=${dataToUpdate['latitude']}, longitude=${dataToUpdate['longitude']}');
      print('🔍 Mise à jour champ - Toutes les données: $dataToUpdate');
      
      print('🔍 Avant db.update - latitude=$latitude (type: ${latitude.runtimeType}), longitude=$longitude (type: ${longitude.runtimeType})');
      print('🔍 dataToUpdate contient: latitude=${dataToUpdate['latitude']}, longitude=${dataToUpdate['longitude']}');
      ...
      if (checkAfterUpdate.isNotEmpty) {
        print('🔍 Vérification immédiate après update - latitude=${checkAfterUpdate.first['latitude']}, longitude=${checkAfterUpdate.first['longitude']}');
      }
      ...
      print('🔍 Vérification DB - latitude=${updated.latitude}, longitude=${updated.longitude}');
  ...
  Future<List<ChampParcelleModel>> getChampsByAdherent(int adherentId) async {
    try {
      ...
      final result = await db.query(
        'champs_parcelles',
        columns: [
          'id', 'adherent_id', 'code_champ', 'nom_champ', 'localisation',
          'latitude', 'longitude', 'superficie', 'type_sol', 'annee_mise_en_culture',
          'etat_champ', 'rendement_estime', 'campagne_agricole', 'variete_cacao',
          'nombre_arbres', 'age_moyen_arbres', 'densite_arbres_associes',
          'systeme_irrigation', 'notes', 'created_at', 'updated_at', 'is_deleted'
        ],
        where: 'adherent_id = ? AND is_deleted = 0',
        whereArgs: [adherentId],
      ...
      for (final row in result) {
        print('🔍 getChampsByAdherent - Données brutes DB pour ${row['code_champ']}: lat=${row['latitude']}, lng=${row['longitude']}');
      }

      final champs = result.map((map) {
        final champ = ChampParcelleModel.fromMap(map);
        print('🔍 getChampsByAdherent - Après fromMap pour ${champ.codeChamp}: lat=${champ.latitude}, lng=${champ.longitude}');
        return champ;
      }).toList();
  ...
  Future<List<ChampParcelleModel>> getAllChampsWithCoordinates() async {
    try {
      ...
      final champs = result.map((map) => ChampParcelleModel.fromMap(map)).toList();
      final champsWithCoords = champs.where((c) => 
        c.latitude != null && 
        c.longitude != null &&
        c.latitude != 0.0 &&
        c.longitude != 0.0 &&
      ...
      print('🔍 getAllChampsWithCoordinates: ${champs.length} champs totaux, ${champsWithCoords.length} avec coordonnées');
  ...
  Future<void> _ensureChampsParcellesColumns(Database db) async {
    try {
      ...
      for (final entry in requiredColumns.entries) {
        if (!columnNames.contains(entry.key)) {
          try {
            ...
            print('✅ Colonne ${entry.key} ajoutée à champs_parcelles (vérification runtime)');
          } catch (e) {
      ...
      print('⚠️ Erreur lors de la vérification des colonnes champs_parcelles: $e');
    }
  }
}
